...
import { APPLICATION_SEARCHABLE_Fields } from "../../constant/application.constant";
import { TApplicationQuery } from "../../interfaces/application.interface";

const GetAppliedJobsService = async (loginCandidateUserId: string, query: TApplicationQuery) => {
    const {
        searchTerm,
        page = 1,
        limit = 10,
    ...
    let searchQuery = {};
    if (searchTerm) {
        searchQuery = makeSearchQuery(searchTerm, APPLICATION_SEARCHABLE_Fields);
    }
...
export default GetAppliedJobsService;